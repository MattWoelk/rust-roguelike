...
impl<'a> System<'a> for VulkanTriangleRenderer {
    ...
    fn setup(&mut self, res: &mut Resources) {
        ...
        // most of the time.
        let instance_ya = self.instance.clone().unwrap();
        ...
        let surface = WindowBuilder::new()
            .build_vk_surface(&events_loop, self.instance.clone().unwrap().clone())
            .unwrap();
...
"
        ...
        if recreate_swapchain {
            ...
            let (new_swapchain, new_images) = match swapchain.recreate_with_dimension(dimensions) {
                ...
                Err(SwapchainCreationError::UnsupportedDimensions) => return, /////continue,
                Err(err) => panic!("{:?}", err),
            };
